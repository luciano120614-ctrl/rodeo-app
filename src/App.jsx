    ...
    <div className="fixed inset-0 z-40 flex flex-col" style={{background:"#ffffff"}}>
      ...
      <div className="px-4 py-2 shrink-0" style={{background:"#ffffff",borderBottom:"1px solid #e5e7eb"}}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={function(){onPausar({fecha,registros:[...log].reverse()});}} style={{background:"#1a2e10"}} className=" border border-gray-200 text-emerald-700 font-bold px-3 py-1.5 rounded-xl text-xs">⏸ Pausar</button>
            <button onClick={function(){onFinalizar({fecha,registros:[...log].reverse()});}} style={{boxShadow:"0 1px 2px rgba(0,0,0,0.1)"}} className="btn-flash bg-red-700 text-white font-black px-4 py-1.5 rounded-xl text-sm border border-red-500">FIN</button>
          </div>
        </div>
      </div>
      {/* Stats bar */}
      ...
      <div className="flex-1 overflow-y-auto flex flex-col">
        ...
        <div className="px-4 pb-4">
          ...
          {log.map(function(r,i){
            return(
              <div key={r.id} className={"flex items-center justify-between rounded-xl px-3 py-2.5 mb-1.5 border "+(i===0?"bg-green-900/30 border-green-600":"bg-gray-50 border-gray-200")}>
                <div>
    ...
    <Modal title="📅 Historial" onClose={onClose}>
      <div className="flex flex-col gap-2">
        ...
        {sorted.map(function(s){
          ...
          return(
            <div key={s.id} style={{background:"#ffffff"}} className=" border border-gray-200 rounded-xl px-4 py-3 flex items-center justify-between">
    ...
    <div className="min-h-screen" style={{background:"#ffffff"}}>
      ...
      <header className="px-4 pt-6 pb-4 border-b border-gray-200">
        <div className="max-w-xl mx-auto flex items-center justify-between">
          ...
          <button onClick={function(){setShowNuevoEst(true);}} style={{boxShadow:"0 1px 3px rgba(0,0,0,0.12)"}} className="btn-flash bg-emerald-600 text-white font-black px-4 py-3 rounded-xl text-sm border border-emerald-500">+ Establecimiento</button>
        </div>
      </header>
